...
def obtener_ruta_salida(ruta_original):
	nombre_imagen_extencion = ruta_original.split("\\")
	nombre_imagen_extencion = nombre_imagen_extencion[len(nombre_imagen_extencion) - 1]
	path_salida = nombre_imagen_extencion.split(".")[0]
	return path_salida + "-salida.png"

def ocultar_texto(mensaje, ruta_imagen_original):
	...
	path_salida = obtener_ruta_salida(ruta_imagen_original)

	if contador >= longitud:
		print("Mensaje escrito correctamente en " + path_salida)
	else:
...
Recibiendo parametros
    sys.argv[1] --> Mensaje
    sys.argv[2] --> Path de la imagen a guardar
'''
if len(sys.argv) == 3:
  ocultar_texto(sys.argv[1], sys.argv[2])
else:
  print("Es necesario pasar un mensaje para ocultar y una imagen.")